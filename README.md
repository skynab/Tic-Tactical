...
- **Scales to the screen it's on**: the UI is laid out against a fixed design resolution (720x1280, set in `project.godot`) and then scaled to whatever size the window actually is, so resizing the window shrinks or grows the whole interface instead of clipping it. Two things make that work:
...
- **Scored win conditions** *(in the New Game dialog)*: win conditions are no longer just "K in a row" — the dialog has a **Win conditions** grid where you tick which shapes count and set how many **points** each is worth. Any ticked shape is a genuine win condition: completing it can take the round on its own. Each row carries a small **diagram** showing the shape as lit cells on a miniature 3x3 board, so the conditions can be told apart without reading the descriptions; the diagram fades out when its condition isn't ticked. The diagrams are illustrative — they always draw a 3x3 so the shapes compare side by side, whatever board size or K value the game is set to.
...
- `PatternIcon.gd` — Draws the small win-condition diagrams in the New Game dialog (a miniature board with the shape's cells lit). Pure `_draw()`, no child nodes.
